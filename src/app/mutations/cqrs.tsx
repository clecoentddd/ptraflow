...
import { validerPlanPaiementCommandHandler } from '@/app/mutations/valider-plan-paiement/handler';
import { executerTransactionCommandHandler } from '@/app/mutations/executer-transaction/handler';
...
export function useCqrs() {
  ...
  const dispatchEvent = (command: AppCommand) => {
    ...
    if ('handler' in command && typeof command.handler === 'function') {
        ...
        handler(context.state, (event: AppEvent) => {
