...
import { preparerDecisionRessourcesCommandHandler } from './handler';
import type { PlanCalculeEvent } from '../calculer-plan/event';

export function preparerDecisionRessourcesProcessManager(state: AppState, event: AppEvent): void {
    ...
    // 2. Check Mutation Type: Is this a RESSOURCES mutation?
    const mutation = queryMutations(state).find(m => m.id === mutationId);
    if (!mutation || mutation.type !== 'RESSOURCES') {
        return;
    }

    // 3. Execute Logic
    console.log(`[ProcessManager] Auto-triggering PREPARER_DECISION_RESSOURCES for mutation ${mutationId}`);
    preparerDecisionRessourcesCommandHandler(state, {
