...
export function ValiderDecisionTodoItem({ mutationId }: { mutationId: string }) {
    const { state } = useCqrs();
    const todos = queryTodos(state);
    const todo = todos.find(t => t.mutationId === mutationId && t.description === 'Valider la décision');
...
export function ValiderDecisionButton({ mutationId }: { mutationId: string }) {
    ...
    const handleClick = () => {
        ...
        if (!decision) {
            toast.error("Impossible de trouver la décision correspondante à valider.");
            return;
        }
